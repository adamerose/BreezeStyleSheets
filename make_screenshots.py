...
if __name__ == '__main__':
    ...
    for theme in ['light', 'dark']:
        ...
        collage_path = f'./screenshots/collage_{theme}.png'
        example = DemoWidget()
        if theme == 'light':
            example.setStyleSheet(qstylish.light())
        elif theme == 'dark':
            example.setStyleSheet(qstylish.dark())
            example.resize(1000, 700)
        ...
        for ix, tab_name in enumerate(
            ...
            pixmap = example.grab()
            save_path = os.path.abspath(os.path.join(screenshots_dir, tab_name + '.jpg'))
            print(save_path)
            pixmap.save(save_path, 'jpg')
        # Make collage

        os.makedirs(screenshots_dir, exist_ok=True)
        image_paths = [os.path.abspath(os.path.join(screenshots_dir, path)) for path in os.listdir(screenshots_dir)]
